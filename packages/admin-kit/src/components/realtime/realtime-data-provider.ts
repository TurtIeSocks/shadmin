...
function locksOrReject<R extends string, T>(
  ...
  fn: (locks: NonNullable<RealtimeDataProviderOptions<R>["locks"]>) => Promise<T>
): Promise<T> {
...
export function realtimeDataProvider<R extends string = string>(
  ...
  const provider: RealtimeDataProvider<R> = {
    ...
    publish<P = unknown>(
      ...
      return transport.publish<P>(topic, event as Parameters<typeof transport.publish<P>>[1]);
    },
    ...
    unlock(resource: R, params: UnlockParams): Promise<Lock> {
      return locksOrReject("unlock", options, (l) => l.unlock(resource, params));
    },

    getLock(resource: R, params: GetLockParams): Promise<Lock | null> {
      return locksOrReject("getLock", options, (l) => l.getLock(resource, params));
    },

    getLocks(resource: R, params?: GetLocksParams): Promise<Lock[]> {
      return locksOrReject("getLocks", options, (l) => l.getLocks(resource, params));
    },

    ...(transport.onReconnect
      ? { onReconnect: (cb: () => void): Unsubscribe => transport.onReconnect!(cb) }
      : {}),
