...
import { Basic } from "@/stories/presence-bar.stories";
...
describe("<PresenceBar />", () => {
  it("renders avatars for other users subscribed to the topic", async () => {
    ...
    await expect.element(presenceBar!.querySelector('[data-slot="avatar-fallback"]:first-of-type') ?? presenceBar!).toBeTruthy();
    // Verify both users' initials appear somewhere in the document
    const fallbacks = presenceBar?.querySelectorAll('[data-slot="avatar-fallback"]');
