...
`<LoginForm>` is also the form rendered by the default [`<LoginPage>`](./login-page) — so customizing the surrounding layout is often as simple as wrapping it differently.
