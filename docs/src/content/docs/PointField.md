...
| --------------- | -------- | ------------------------ | ---------------------- | -------------------------------------------- |
| `source`        | Required | `string`                 | -                      | Record field holding the `Point` geometry.   |
...
| `pathOptions`   | Optional | `L.PathOptions`          | `{ color: "#3388ff" }` | Vector styling (unused for Point markers).   |
| `markerIcon`    | Optional | `L.Icon \| L.DivIcon`    | shared `MarkerIcon`    | Icon used for the Point marker.              |
| `fitBounds`     | Optional | `boolean`                | `true`                 | Auto-fit map view to the geometry on mount.  |
