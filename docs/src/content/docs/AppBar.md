...
`<AppBar>` is included in the default [`<Layout>`](./Layout.md). It can be re-used when building a custom layout, or replaced entirely by passing children.
...
When `children` are provided, they replace the entire toolbar — `<AppBar>` only contributes the `<header>` wrapper with its default flex layout. Compose with [`<TitlePortal>`](./TitlePortal.md), [`<SidebarToggleButton>`](./SidebarToggleButton.md), [`<UserMenu>`](./UserMenu.md), and other building blocks to keep parity with the default behavior.
