...
Renders a page title inside the [`<TitlePortal>`](./title-portal) slot in the app bar.

`<Title>` teleports its content into the `ra-title-portal` element rendered by `<TitlePortal>`. The default [`<Layout>`](./layout) includes this portal inside [`<AppBar>`](./app-bar), so any view can declare its own page title without knowing where it appears in the DOM.
