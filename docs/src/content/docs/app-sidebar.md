...
`<AppSidebar>` is included in the default [`<Layout>`](./layout) component. It includes:

- the application name and logo, whicj is a link to the home page (`/`),
- a link to the dashboard (if defined in [`<Admin dashboard>`](./admin)), and
- links to all the list views defined in [`<Resource list>`](./resource) components.
...
To reorder the Resource menu items, reorder the `<Resource>` components in the [`<Admin>`](./admin) component.
...
To customize the menu items, edit the `SidebarContent` section. For example, if you added a `settings` [`CustomRoute`](./custom-routes), here is how to add a link to it in the sidebar:
