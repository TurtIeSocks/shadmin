...
export const PostList = () => (
  <List pagination={<PostListPagination />}>{/* ... */}</List>
);
```

Other components that create a `ListContext`, like [`<ReferenceManyField>`](./reference-many-field), don't include pagination by default. You can use `<ListPagination>` to add pagination to them:
