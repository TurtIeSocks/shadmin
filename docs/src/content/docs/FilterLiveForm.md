...
Form whose inputs push their values to the surrounding [`<ListContext>`](https://marmelab.com/react-admin/useListContext.html) filter state on every change.

`<FilterLiveForm>` is re-exported from `ra-core` for convenience and pairs nicely with [`<FilterList>`](./FilterList.md) and [`<FilterListSection>`](./FilterListSection.md).

## Usage

Drop a `<FilterLiveForm>` inside any container that exposes a `ListContext` (typically the `aside` of a [`<List>`](./List.md) view) and wrap one or more filter inputs:
