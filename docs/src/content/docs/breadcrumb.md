...
`<Breadcrumb>` defines a breadcrumb path and renders it in the `<div id="breadcrumb">` portal, located in the header of the default [`<Layout>`](./layout) component.

Default page components ([`<List>`](./list), [`<Show>`](./show), [`<Edit>`](./edit), [`<Create>`](./create)) automatically set the breadcrumb based on the current resource and action. For detail pages, it uses the [`<Resource recordRepresentation>`](./resource) to display the current record.
...
If your application defines a custom [`<Admin layout>`](./admin) component, make sure to include a `<div id="breadcrumb">` element where you want the breadcrumb to be rendered.
