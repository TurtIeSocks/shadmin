...
To fetch the options from another resource, wrap `<SelectArrayInput>` in a [`<ReferenceArrayInput>`](./ReferenceArrayInput.md):
...
- [`<CheckboxGroupInput>`](./CheckboxGroupInput.md) for an inline list of checkboxes (better for short lists)
- [`<AutocompleteArrayInput>`](./AutocompleteArrayInput.md) for a searchable autocomplete (better for very long lists)
