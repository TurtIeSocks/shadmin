...
Render a list of records as a vertical, mobile-friendly list — typically used as a child of [`<List>`](./list) or [`<ReferenceManyField>`](./reference-many-field).
...
What to render when the list is empty. Defaults to [`<ListNoResults>`](./list-no-results):
...
What to render while the list is loading. Defaults to a [`<SimpleListLoading>`](./simple-list-loading) skeleton sized after the configured slots.
