...
`<TranslatableFields>` lets you display the same field in several languages, exposing one tab per locale. It is the read-only counterpart of [`<TranslatableInputs>`](./translatable-inputs).
