...
An infinite-scroll variant of [`<List>`](./List.md), backed by `useInfiniteListController` from `ra-core`.
...
It pairs nicely with [`<SimpleList>`](./SimpleList.md) for mobile-friendly screens:
...
By default, `<InfiniteList>` renders an [`<InfinitePagination>`](./InfinitePagination.md). Override it to customize the loader or hide it entirely:
