...
Use it inside any component creating a `ListContext` (e.g. [`<ArrayField>`](./array-field), [`<ReferenceArrayField>`](./reference-array-field), [`<ReferenceManyField>`](./reference-many-field)).
...
`<SingleFieldList>` creates one `RecordContext` per item in the list. By default, it renders each item as a badge, using [`<BadgeField>`](./badge-field) and the resource [`recordRepresentation`](https://marmelab.com/ra-core/resource/#recordrepresentation).
