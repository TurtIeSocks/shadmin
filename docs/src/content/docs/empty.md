...
Default empty state rendered by [`<List>`](./list) when the data provider returns zero records and there are no active filters.
...
`<Empty>` introspects the current resource via `useResourceContext()` and `useResourceDefinition()`. When the resource defines a `create` route, an invitation message and a [`<CreateButton>`](./create-button) are displayed.
