...
It is the same shell used by the default [`<LoginPage>`](./LoginPage.md), and is intentionally minimal — you control what is rendered in the right pane through `children`.
...
The content rendered in the right pane. Usually a form component such as [`<LoginForm>`](./LoginForm.md) or [`<LoginWithEmail>`](./LoginWithEmail.md).
