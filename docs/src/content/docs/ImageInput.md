...
`<ImageInput>` allows editing and uploading image attachments. It is a thin wrapper around [`<FileInput>`](./FileInput.md) that defaults to accepting only images and renders an `<ImageField>` thumbnail preview by default.
...
`<ImageInput>` accepts the same props as [`<FileInput>`](./FileInput.md):
