...
Full-page error screen displayed when authentication fails — typically because the user's session has expired or their credentials are no longer valid. Mirrors [`<AccessDenied>`](./AccessDenied.md) but uses a warning triangle icon and different default texts.
