...
Render a list of records as a vertical, mobile-friendly list — typically used as a child of [`<List>`](./List.md) or [`<ReferenceManyField>`](./ReferenceManyField.md).
...
What to render while the list is loading. Defaults to a [`<SimpleListLoading>`](./SimpleListLoading.md) skeleton sized after the configured slots.
