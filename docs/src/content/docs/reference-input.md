...
`<ReferenceInput>` renders an [`<AutocompleteInput>`](./autocomplete-input) to let the user select the related record. Users can narrow down the choices by typing a search term in the input. This modifies the query sent to the `dataProvider` as follows:
...
You can replace the default `<AutocompleteInput>` by another choice input. To do so, pass the choice input component as `<ReferenceInput>` child. For instance, to use a [`<SelectInput>`](./select-input):
...
By default, `<ReferenceInput>` renders an [`<AutocompleteInput>`](./autocomplete-input) to let end users select the reference record.
...
You can also use [`<SelectInput>`](./select-input) or [`<RadioButtonGroupInput>`](./radio-button-group-input) instead of [`<AutocompleteInput>`](./autocomplete-input).
...
For instance, if you want to transform an option value before rendering, and the selection control is an `<AutocompleteInput>` (the default), set [the `<AutocompleteInput format>` prop](./autocomplete-input) as follows:
...
For instance, if you want to transform an option value before submission, and the selection control is an `<AutocompleteInput>` (the default), set [the `<AutocompleteInput parse>` prop](./autocomplete-input) as follows:
...
For instance, to make the reference required, and the selection control is an `<AutocompleteInput>` (the default), set [the `<AutocompleteInput validate>` prop](./autocomplete-input) as follows:
...
If you want to customize the conversion between the search term and the query filter to match the filtering capabilities of your API, use the [`<AutocompleteInput filterToQuery>`](./autocomplete-input) prop.
