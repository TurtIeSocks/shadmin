...
To fetch the options from another resource, wrap `<CheckboxGroupInput>` in a [`<ReferenceArrayInput>`](./ReferenceArrayInput.md):
...
- [`<SelectArrayInput>`](./SelectArrayInput.md) for a dropdown multi-select
- [`<AutocompleteArrayInput>`](./AutocompleteArrayInput.md) for a searchable autocomplete
