...
A drop-in replacement for [`<SimpleForm>`](./simple-form) whose visible inputs can be reordered or hidden by the end user via the [`<Inspector>`](./inspector).
...
When the user toggles edit mode (via the `<InspectorButton>` in the app bar), a cogwheel appears next to the form. Clicking it opens the inspector with a [`<FieldsSelector>`](./inspector) of the form's inputs. Users can toggle visibility and drag rows to reorder.
...
| ...rest         | -        | `SimpleFormProps` | -                        | All other props are forwarded to the underlying [`<SimpleForm>`](./simple-form). |
...
Internally, `<SimpleFormConfigurable>` wraps `<SimpleForm>` in a [`<Configurable>`](./inspector) whose editor is a `<FieldsSelector>` reading from `inputs` / `availableInputs` preference keys.
...
- [`<Inspector>`](./inspector) for the full configurable system
- [`<SimpleForm>`](./simple-form) for the underlying form layout
