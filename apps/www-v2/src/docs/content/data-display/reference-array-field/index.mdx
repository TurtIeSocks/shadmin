...
description: "Fetches multiple referenced records by an array of ids contained in the current record, and provides them through a ListContext to its children. Use it to display a list of related records, via a one-to-many relationship materialized by an array of foreign keys."
registry: "reference-array-field"
---
...
By default, `<ReferenceArrayField>` renders one string by related record, via a [`<SingleFieldList>`](/docs/data-display/single-field-list) with a [`<BadgeField>`](/docs/data-display/badge-field) child using the resource [`recordRepresentation`](https://marmelab.com/ra-core/resource/#recordrepresentation) as source.
...
- Use [`<ReferenceManyField>`](/docs/data-display/reference-many-field) instead when you need to display records from another resource that reference the current record (inverse relationship).
