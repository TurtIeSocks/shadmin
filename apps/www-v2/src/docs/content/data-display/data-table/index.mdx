---
title: "DataTable"
description: "A feature-rich data table with sortable columns, row selection, bulk actions, column visibility, and conditional formatting."
registry: "data-table"
---
...
- Column visibility & reordering (via [`ColumnsButton`](/docs/data-display/columns-button))
- Pagination (via [`ListPagination`](/docs/data-display/list-pagination))
...
Use `<DataTable>` inside a `ListContext` (e.g., as a descendent of [`<List>`](/docs/page-components/list) or [`<ReferenceManyField>`](/docs/data-display/reference-many-field)). Define the table columns with its children using `<DataTable.Col>` components:
...
- [`<BulkDeleteButton>`](/docs/data-edition/bulk-delete-button) (enabled by default)
- [`<BulkExportButton>`](/docs/data-edition/bulk-export-button) to export only the selection
...
You can let end users customize the fields displayed in the `<DataTable>` by using the [`<ColumnsButton>`](/docs/data-display/columns-button) in the `<List actions>`. When users click on this button, they can show / hide columns and reorder them.
...
If you include a [`<ColumnsButton>`](/docs/data-display/columns-button) in a page that has more than one `<DataTable>`, you have to link the two components by giving them the same `storeKey`:
