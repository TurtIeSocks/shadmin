...
description: "Form control that tets users choose a value in a list using a dropdown with autocompletion. This input allows editing record fields that are scalar values, e.g. 123, 'admin', etc."
registry: "autocomplete-input"
---
...
- [`<SelectInput>`](/docs/data-edition/select-input) renders a dropdown
- [`<RadioButtonGroupInput>`](/docs/data-edition/radio-button-group-input) renders a list of radio buttons

:::

:::tip
If you need to let users select more than one item in the list, check out the [`<AutocompleteArrayInput>`](/docs/data-edition/autocomplete-array-input) component.
:::
