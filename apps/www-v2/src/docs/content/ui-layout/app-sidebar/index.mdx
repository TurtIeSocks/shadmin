---
title: "AppSidebar"
description: "Navigation sidebar displaying menu items, allowing users to navigate between different sections of the application. They can collapse it to an icon-only sidebar by clicking on the sidebar icon in the breadcrumb. On mobile, it renders as a collapsible drawer. <AppSidebar> leverages shadcn/ui's Sidebar component."
registry: "app-sidebar"
---

![AppSidebar](/docs/images/app-sidebar.jpg)

## Usage

`<AppSidebar>` is included in the default [`<Layout>`](/docs/ui-layout/layout) component. It includes:

- the application name and logo, whicj is a link to the home page (`/`),
- a link to the dashboard (if defined in [`<Admin dashboard>`](/docs/app-config/admin)), and
- links to all the list views defined in [`<Resource list>`](/docs/app-config/resource) components.
...
To reorder the Resource menu items, reorder the `<Resource>` components in the [`<Admin>`](/docs/app-config/admin) component.
...
To customize the menu items, edit the `SidebarContent` section. For example, if you added a `settings` [`CustomRoute`](/docs/app-config/custom-routes), here is how to add a link to it in the sidebar:
