---
title: "Empty"
description: "Default empty state rendered by <List> when the data provider returns zero records and there are no active filters."
registry: "empty"
---
...
`<Empty>` introspects the current resource via `useResourceContext()` and `useResourceDefinition()`. When the resource defines a `create` route, an invitation message and a [`<CreateButton>`](/docs/data-edition/create-button) are displayed.
