...
The `<Edit>` component will render its children inside a `EditContext` provider, which the `save` function. Children can be any React node, but are usually a form component like [`<SimpleForm>`](/docs/data-edition/simple-form), or the headless [`<Form>`](https://marmelab.com/ra-core/form/) component.
...
- [`<CreateButton>`](/docs/data-edition/create-button) to create a new record
- [`<ListButton>`](/docs/ui-layout/list-button) to go back to the list
- [`<ShowButton>`](/docs/data-edition/show-button) to go to the show page
- [`<DeleteButton>`](/docs/data-edition/delete-button) to delete the current record
...
:::tip
If you want to have different transformations based on the button clicked by the user (e.g. if the creation form displays two submit buttons, one to "save", and another to "save and notify other admins"), you can set the `transform` prop on [the `<SaveButton>` component](/docs/data-edition/save-button), too.
:::
