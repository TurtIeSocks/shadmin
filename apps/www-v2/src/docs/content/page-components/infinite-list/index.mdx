---
title: "InfiniteList"
description: "An infinite-scroll variant of <List>, backed by useInfiniteListController from ra-core."
registry: "infinite-list"
---
...
It pairs nicely with [`<SimpleList>`](/docs/page-components/simple-list) for mobile-friendly screens:
...
When the list has no records and no active filters, `<InfiniteList>` renders the default [`<Empty>`](/docs/ui-layout/empty) component. Pass a custom node to replace it, or `false` to suppress:
...
By default, `<InfiniteList>` renders an [`<InfinitePagination>`](/docs/data-display/infinite-pagination). Override it to customize the loader or hide it entirely:
