---
title: "I18N"
description: "Translate the admin UI and let users switch locales with the i18n provider."
---

<video controls autoPlay playsInline muted loop className="w-full aspect-720/420">
    <source src="/docs/images/translation.mp4" type="video/mp4" />
    Your browser does not support the video tag.
</video>

The Shadmin user interface uses English as the default language. But you can also display the UI and content in other languages, allow changing language at runtime, and even lazy-loading optional languages to avoid increasing the bundle size with all translations.

Shadmin supports [over 35 translations](https://marmelab.com/ra-core/translationlocales/) out of the box, and you can [update or complete them](https://marmelab.com/ra-core/translationtranslating/) as needed.
...
Just like for data fetching and authentication, Shadmin is agnostic to your translation backend. It relies on a provider for internationalization. It's called the `i18nProvider`, and it manages translation and language changes.
...
Use the `<Admin i18nProvider>` prop to define the `i18nProvider` of a Shadmin app:
...
Shadmin components use translation keys for their text and rely on the `i18nProvider` to translate them.
...
Although you can build an `i18nProvider` from scratch, Shadmin provides a package called `ra-i18n-polyglot` that leverages [the Polyglot.js library](https://airbnb.io/polyglot.js/) to build an `i18nProvider` based on a dictionary of translations.
...
Shadmin also provides a package called `ra-i18n-i18next` that leverages [the i18next library](https://www.i18next.com/) to build an `i18nProvider` based on a dictionary of translations.
...
// in src/App.tsx
import { Admin } from '@/components/admin/admin';
import { useMyI18nProvider } from './i18n-provider';
...
When building an internationalized app with Shadmin, the usual workflow is therefore to let developers write the main translation file. Then, pass this file to a team of translators, with the task to translate it. They can use any software they want for that (even software using Gettext/PO files, as it's possible to convert POT to and from JSON). Finally, aggregate all the translations into an `i18nProvider`.

Check [the translation setup documentation](https://marmelab.com/ra-core/translationsetup/) to understand how to build your own translation file, the [list of available translations](https://marmelab.com/ra-core/translationlocales/) to find a translation for your language, and [Translating the UI](https://marmelab.com/ra-core/translationtranslating/) to understand how to translate Shadmin components.

## Localization

For numeric and temporal values, Shadmin benefits from the Single-Page Application architecture. As the application executes in the browser, it uses the browser's locale by default to format numbers and dates.
