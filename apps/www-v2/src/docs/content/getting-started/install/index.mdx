---
title: "Installation"
description: "shadmin is a React library of components for admin apps. You can install it with any React framework:"
---

- [Install with Next.js](#install-with-nextjs)
- [Install with Vite.js](#install-with-vitejs)
...
Next, go to your project directory and pull the `shadmin` components using the `shadcn` command:
...
Next step: Read the [Quick Start Guide](/docs/getting-started/quick-start-guide) to learn how to use the components in your admin app.

## Install with Vite.js

`shadmin` is built on React, so you need to create a Vite single-page app to use it.
...
You can now start adding components to your project. Let's start with the `shadmin` components.
...
The `<App>` component currently renders a default Vite application. You can replace all its content by the following to serve your new `shadmin` application.
...
You can setup a Shadmin application with React-Router v7 (a.k.a. Remix v3) as follows. First, creating a new React Router project with the following command:
...
Next, install the Shadmin components:
...
:::note
The convention for react-router applications is to use `~/` as prefix for imports, while other frameworks use `@/`. The Shadmin documentation uses `@/` everywhere, make sure to adapt the import paths accordingly.
:::
...
You can setup a Shadmin application with Tanstack Start as follows. First, create a new Tanstack Start project with the following command:
...
Now that shadcn is set up, let's start with the `shadmin` components.

```shell
npx shadcn@latest add https://shadmin.turtlesocks.dev/r/admin.json
```

Last but not least, you need to add `ra-router-tanstack` to your project, so that your `shadmin` application can make use of the tanstack router:
...
When you run `npm run dev` and visit [http://localhost:3000](http://localhost:3000/), you should see the Shadmin welcome screen:

![Welcome to shadmin!](/docs/images/welcome-tanstack-start.png)
