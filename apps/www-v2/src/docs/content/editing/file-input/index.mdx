...
`<FileInput>` uses its child component to give a preview of the files. `<FileInput>` renders its child once per file, inside a `<RecordContext>`, so the child can be a Field component. The default [`<FileField>`](/docs/viewing/file-field) renders the name of the file(s), with a hyperlink.
...
You can use `<FileInput>` to upload images as well. Set the `accept` prop to accept only image files, and use [`<ImageField>`](/docs/viewing/image-field) as the child component to show image previews.
