...
- Column visibility & reordering (via [`ColumnsButton`](/docs/viewing/columns-button))
- Pagination (via [`ListPagination`](/docs/viewing/list-pagination))
...
Use `<DataTable>` inside a `ListContext` (e.g., as a descendent of [`<List>`](/docs/page-components/list) or [`<ReferenceManyField>`](/docs/viewing/reference-many-field)). Define the table columns with its children using `<DataTable.Col>` components:
