...
`<SimpleForm>` is often used as child of [`<Create>`](./create) or [`<Edit>`](./edit). It accepts Input components as children. It requires no prop by default.
...
The basic usage of `<SimpleForm>` is to pass Input components as children. For non-editable fields, you can pass `readOnly` inputs, or even a [`<RecordField>`](./record-field) component.
