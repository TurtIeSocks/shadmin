...
- Column visibility & reordering (via [`ColumnsButton`](./columns-button))
- Pagination (via [`ListPagination`](./list-pagination))
...
Use `<DataTable>` inside a `ListContext` (e.g., as a descendent of [`<List>`](./list) or [`<ReferenceManyField>`](./reference-many-field)). Define the table columns with its children using `<DataTable.Col>` components:
...
If you include a [`<ColumnsButton>`](./columns-button) in a page that has more than one `<DataTable>`, you have to link the two components by giving them the same `storeKey`:
