...
Displays the user's current subscription plan as a compact badge showing the
plan name and price. Looks up the matched plan in a supplied `plans` array by
record id.
...
[`<SubscriptionPlanPicker>`](./subscription-plan-picker.md)).
