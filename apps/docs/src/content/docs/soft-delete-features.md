...
Once your provider has all soft-delete methods, pass it to the [`<Admin>`](./admin) component and you're ready to start using the Soft Delete feature.
...
The [`<BulkSoftDeleteButton>`](./bulk-delete-button.md#soft-delete) has to create or update several records at once. If your data provider supports the `createMany` method to create multiple records at once, it will use it instead of calling `create` multiple times for performance reasons.
