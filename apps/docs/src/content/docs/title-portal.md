...
Render target for [`<Title>`](./title). Reserves the DOM slot in the app bar where page titles are teleported.

The default [`<Layout>`](./layout) includes a `<TitlePortal>` inside [`<AppBar>`](./app-bar). Re-use it when composing a custom header.
...
Pair it with [`<Title>`](./title) (rendered anywhere in the page tree) to populate the slot:
