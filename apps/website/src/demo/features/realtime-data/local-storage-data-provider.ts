...
type Store = { [resource: string]: Row[] };

/**
 * Demo-only data provider that persists state in localStorage so multiple
 * browser tabs share a single source of truth. Pairs with the realtime
 * transport: mutations broadcast events, peer tabs invalidate and refetch —
 * and the refetch hits the same localStorage snapshot the mutating tab wrote.
 */
export function localStorageDataProvider(
  seed: Store,
  storageKey = "shadmin-demo-realtime-data",
): DataProvider {
  function read(): Store {
  ...
  function collection(store: Store, resource: string): Row[] {
  ...
  function nextId(records: Row[]): number {
  ...
  function matchesFilter(record: Row, filter: Row): boolean {
  ...
  return {
    async getList(resource, params) {
      ...
      const filtered = params.filter
        ? all.filter((r) => matchesFilter(r, params.filter as Row))
        : all;
    ...
    async create(resource, params) {
      ...
      const record: Row = { ...data, id: data.id ?? nextId(records) };
      records.push(record);
      write(store);
      return { data: record as never };
    },
    ...
    async updateMany(resource, params) {
      const store = read();
      const records = collection(store, resource);
      const patch = params.data as Row;
