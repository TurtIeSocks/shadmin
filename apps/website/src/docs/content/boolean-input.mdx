...
:::tip
This input doesn't let users set a `null` value - only `true` or `false`. Use the [`<SelectInput>`](./select-input) component if you have to handle non-required booleans.
:::
