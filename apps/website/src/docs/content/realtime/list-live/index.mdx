---
title: "ListLive"
description: "<ListLive> is a drop-in replacement for <List> that subscribes to realtime events for the resource. When a created, updated, or deleted event arrives on the resource/<name> topic — or when the transport reconnects — the list is automatically refreshed."
registry: "list-live"
---
...
`<ListLive>` accepts every prop that `<List>` accepts. See the [`<List>` documentation](/docs/page-components/list/overview) for the full prop reference.
