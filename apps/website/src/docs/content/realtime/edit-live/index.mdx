---
title: "EditLive"
description: "<EditLive> is a drop-in replacement for <Edit> that subscribes to realtime events for the current record. When an updated or deleted event arrives on resource/<name>/<id> — or when the transport reconnects — the record is automatically re-fetched."
registry: "edit-live"
---
...
`<EditLive>` accepts every prop that `<Edit>` accepts. See the [`<Edit>` documentation](/docs/page-components/edit/overview) for the full prop reference.
