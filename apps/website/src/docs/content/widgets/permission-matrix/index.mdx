...
description: "<PermissionMatrix> renders an editable role × resource × action grid. Each cell is a checkbox: \"can <role> do <action> on <resource>?\". It reads and writes via a controlled value/onChange API and is compatible with any auth model — you supply the role list, resource list, action list, and current state."
