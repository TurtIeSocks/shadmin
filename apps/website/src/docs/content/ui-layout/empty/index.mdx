...
`<Empty>` introspects the current resource via `useResourceContext()` and `useResourceDefinition()`. When the resource defines a `create` route, an invitation message and a [`<CreateButton>`](/docs/editing/create-button) are displayed.
