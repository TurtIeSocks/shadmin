...
description: "A compact icon-only button that refreshes the current view's data. Wraps the icon in a tooltip with the localized label."
registry: "refresh-icon-button"
---
...
For a full text button, use [`<RefreshButton>`](/docs/ui-layout/refresh-button) instead.
