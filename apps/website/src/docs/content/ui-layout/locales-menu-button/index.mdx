...
description: "Also known as the \"language switcher\", it displays a menu allowing users to select the language of the interface."
registry: "locales-menu-button"
---

![LocalesMenuButton](/docs/images/locales-menu-button.jpg)

It leverages the [store](https://shadmin.turtlesocks.dev/Store.html) so that their selection is persisted.

## Usage

For most users, this component will be automatically added to the header of shadmin's default [`<Layout>`](/docs/ui-layout/layout) if the `i18nProvider.getLocales()` method returns more than one locale. Check the [`i18nProvider` setup documentation](https://marmelab.com/ra-core/translationsetup/) for details.
