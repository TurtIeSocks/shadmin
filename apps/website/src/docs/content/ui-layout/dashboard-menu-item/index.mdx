...
description: "Sidebar entry linking to the dashboard (/). Uses the default house icon and the translated label ra.page.dashboard."
registry: "dashboard-menu-item"
---

## Usage

The default [`<Menu>`](/docs/ui-layout/menu) renders `<DashboardMenuItem>` automatically when an `<Admin dashboard>` is configured.
