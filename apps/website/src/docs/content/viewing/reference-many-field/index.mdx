...
description: "Fetches multiple referenced records that reference the current record, and provides them through a ListContext to its children. Useful for displaying a list of related records via a one-to-many relationship, when the foreign key is carried by the referenced resource."
registry: "reference-many-field"
---
