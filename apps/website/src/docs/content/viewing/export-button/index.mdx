...
It relies on [the `exporter` function](/docs/page-components/list/data-export#exported-data) passed to the `<List>` component, via the `ListContext`. It's disabled for empty lists.
