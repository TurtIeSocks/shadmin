---
title: "FilterButton"
description: "Dropdown that lets users add, remove, and manage the filters displayed in the top <FilterForm> of a <List> view."
...
Most of the time, `<FilterButton>` reads the filter list from the surrounding [`<FilterContext>`](https://marmelab.com/react-admin/useFilterContext.html), which is populated by [`<List filters>`](/docs/page-components/list/filtering-sorting#filter-button--form-combo). You can pass an explicit `filters` array when rendering `<FilterButton>` outside that context.
