...
`<FilterLiveForm>` is re-exported from `ra-core` for convenience and pairs nicely with [`<FilterList>`](/docs/viewing/filter-list) and [`<FilterListSection>`](/docs/viewing/filter-list-section).

## Usage

Drop a `<FilterLiveForm>` inside any container that exposes a `ListContext` (typically the `aside` of a [`<List>`](/docs/page-components/list/overview) view) and wrap one or more filter inputs:
