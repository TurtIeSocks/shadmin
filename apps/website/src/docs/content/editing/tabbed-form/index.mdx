---
title: "TabbedForm"
description: "<TabbedForm> creates a <form> to edit a record with inputs organized into tabs, and a toolbar at the bottom (Cancel + Save buttons). It renders all tab panels simultaneously (hidden via CSS when inactive) so form validation runs across every tab, not just the visible one."
registry: "tabbed-form"
---

## Usage

`<TabbedForm>` is often used as child of [`<Create>`](/docs/page-components/create) or [`<Edit>`](/docs/page-components/edit/overview). Use `<TabbedForm.Tab>` components to define each tab.
