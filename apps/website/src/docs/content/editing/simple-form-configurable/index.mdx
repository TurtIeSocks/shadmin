...
description: "A drop-in replacement for <SimpleForm> whose visible inputs can be reordered or hidden by the end user via the <Inspector>."
registry: "simple-form-configurable"
---
...
When the user toggles edit mode (via the `<InspectorButton>` in the app bar), a cogwheel appears next to the form. Clicking it opens the inspector with a [`<FieldsSelector>`](/docs/ui-layout/inspector) of the form's inputs. Users can toggle visibility and drag rows to reorder.
...
| ...rest         | -        | `SimpleFormProps` | -                        | All other props are forwarded to the underlying [`<SimpleForm>`](/docs/editing/simple-form/overview). |
...
Internally, `<SimpleFormConfigurable>` wraps `<SimpleForm>` in a [`<Configurable>`](/docs/ui-layout/inspector) whose editor is a `<FieldsSelector>` reading from `inputs` / `availableInputs` preference keys.
...
- [`<Inspector>`](/docs/ui-layout/inspector) for the full configurable system
- [`<SimpleForm>`](/docs/editing/simple-form/overview) for the underlying form layout
