...
`<ReferenceArrayInput>` renders an [`<AutocompleteArrayInput>`](/docs/editing/autocomplete-array-input) to let the user select the related record. Users can narrow down the choices by typing a search term in the input. This modifies the query sent to the `dataProvider` as follows:
...
For instance, if you want to transform an option value before rendering, and the selection control is an `<AutocompleteArrayInput>` (the default), set [the `<AutocompleteArrayInput format>` prop](/docs/editing/autocomplete-array-input) as follows:
...
For instance, if you want to transform an option value before submission, and the selection control is an `<AutocompleteArrayInput>` (the default), set [the `<AutocompleteArrayInput parse>` prop](/docs/editing/autocomplete-array-input) as follows:
...
For instance, to make the selection required, and the selection control is an `<AutocompleteArrayInput>` (the default), set [the `<AutocompleteArrayInput validate>` prop](/docs/editing/autocomplete-array-input) as follows:
...
If you want to customize the conversion between the search term and the query filter to match the filtering capabilities of your API, use the [`<AutocompleteArrayInput filterToQuery>`](/docs/editing/autocomplete-array-input) prop.
