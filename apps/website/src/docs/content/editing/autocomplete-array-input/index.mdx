...
:::tip
`<AutocompleteArrayInput>` is a stateless component, so it only allows to _filter_ the list of choices, not to _extend_ it. If you need to populate the list of choices based on the result from a `fetch` call (and if [`<ReferenceArrayInput>`](/docs/editing/reference-array-input) doesn't cover your need), you'll have to write your own Input component.
:::
